...
t_err_00 = []
t_err_22 = []
times = np.array(np.arange(10, 300, 10))
...
for t in times:  
    ...
    if model.means_[0][0] + model.means_[0][1] > model.means_[1][0] + model.means_[1][1]:
        ...
        t_err_00.append(((model.means_[0][0] - 0)**2 + (model.means_[0][1] - 0)**2)**0.5)
        t_err_22.append(((model.means_[1][0] - 2)**2 + (model.means_[1][1] - 2)**2)**0.5)

plt.plot(times, t_err_00, label='center at (0, 0)')
plt.plot(times, t_err_22, label='center at (2, 2)')
